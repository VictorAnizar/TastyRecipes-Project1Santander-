    ...
    getCategories().then((data) => {
        ...
        for (let iterator = 0; iterator < data.categories.length; iterator++) {
            section_categoraias.append(
                ...
                creaTarjetaReceta(
                    data,
                    data.categories[iterator].strCategory,
                    data.categories[iterator].strCategoryThumb,
                    data.categories[iterator].strCategoryDescription,
                    iterator,
                    "categoria"
                )
            );
...
function logicaBuscador() {
    ...
    btn_buscar.addEventListener("click", function () {
        if (search_input.value != "") {
            ...
            document
                .querySelector("#div_recipe_random")
                .classList.add("closed");

            getRecetaSearch(search_input.value)
                .then((data) => {
                    document.querySelector("#title_section_lista").innerHTML =
                        " " +
                        data.meals.length +
                        ' resultados para "' +
                        search_input.value +
...
function escuchadoresCloseModal() {
    ...
    modal.addEventListener("click", function () {
        ...
        document.querySelector(".modal-content").style.animationName =
            "modal-close";
    });
}
...
function recetaSorpresa() {
    getRecetaRandom()
        ...
        .then(function (div_card) {
            const recetaSorpresaAleatoria =
                document.querySelector(".receta-sorpresa");
...
function creaTarjetaReceta(
    ...
    div_card.addEventListener("click", function () {
        ...
        if (indicador == "receta") {
            ...
            modalActive.innerHTML = `<div id="image-title-modal">
                <img src="${imagen}"/>
                <h1>
                   ${nombre} 
                </h1>
            ...
            modalActive.style.backgroundImage =
                'url("' + backgroundModal + '")';
            ...
            getSearchByCategory(nombre).then((data) => {
